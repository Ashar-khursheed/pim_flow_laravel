...
if (!function_exists('deleteS3FileFromUrl')) {
	function deleteS3FileFromUrl($fileUrl)
	{
		if (!$fileUrl || !Str::startsWith($fileUrl, [env('AWS_URL'), env('AWS_CACHE_URL')])) {
			return;
		}

		$existingPath = Str::after($fileUrl, env('AWS_URL') . '/');

		if ($existingPath === $fileUrl) {
			$existingPath = Str::after($fileUrl, env('AWS_CACHE_URL') . '/');
		}
		...
		try {
			...
			Log::warning("Failed to delete S3 file: {$e->getMessage()}", [
				'file_url' => $fileUrl,
				'extracted_path' => $existingPath,
			]);
		}
	}
}
