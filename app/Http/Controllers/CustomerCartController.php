    ...
    try {
        ...
        if (!$customerCart) {
            $customerCart = CustomerCart::create([
                ...
                'customer_address_id' => $request->customer_address_id,
                'shipping_charge' => 0,
                'is_lift_gate' => $request->is_lift_gate ?? 0,
                'is_residential_address' => $request->is_residential_address ?? 1,
                'amount' => 0,
                'tax_percentage' => $request->tax_percentage,
                'tax_amount' => 0,
                'total_amount' => 0,
        ...
        foreach ($request->products as $item) {
            $product = Product::with('productSuppliers')->find($item['product_id']);
            $supplier = $product->productSuppliers->where('vendor_id', $item['vendor_id'])->first()
                        ?? $product->productSuppliers->first();

            if (!$supplier) {
                throw new \Exception("Supplier not found for product ID {$item['product_id']}");
            }
            ...
            $cartProduct = CustomerCartProduct::updateOrCreate(
                [
                    'customer_cart_id' => $customerCart->id,
        ...
        // Add lift gate / residential charges
        $cartAmount += $request->boolean('is_lift_gate') ? 75 : 0;
        $cartAmount += $request->boolean('is_residential_address') ? 199 : 0;
        ...
        // Handle new customer password
        if ($request->boolean('is_new_customer')) {
            $randomPassword = Str::random(8);
            $customerCart->customer->update(['password' => Hash::make($randomPassword)]);
        } else {
            $randomPassword = null;
        }
