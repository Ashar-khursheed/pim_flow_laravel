 ...
 *             @OA\Property(property="primary_keyword", type="string", example="best product"),
 *             @OA\Property(property="monthly_search_volume", type="integer", example=1000),
 *             @OA\Property(property="title_tag", type="string", example="Updated Title Tag"),
 *             @OA\Property(property="meta_title", type="string", example="Updated Meta Title"),
 *             @OA\Property(property="meta_description", type="string", example="Updated description"),
 *             @OA\Property(property="indexing", type="boolean", example=true),
 *             @OA\Property(property="schema_rating", type="integer", example=4),
 *             @OA\Property(property="schema_reviews_count", type="integer", example=10),
 *             @OA\Property(property="tags", type="string", example="new, updated, seo"),
 *             @OA\Property(
 *                 property="secondary_keywords",
 *                 type="array",
 *                 @OA\Items(
 *                     type="object",
 *                     @OA\Property(property="secondary_keyword", type="string", example="secondary key"),
 *                     @OA\Property(property="monthly_search_volume", type="integer", example=300)
 *                 )
 *             )
 *         )
 *     ),
 ...
 */
    ...
    public function update(Request $request, $id)
    {
        ...
        $seo->update($request->only([
            'relational_id', 'relational_type', 'url', 'primary_keyword', 'monthly_search_volume',
            'title_tag', 'meta_title', 'meta_description', 'internal_links', 'indexing',
            'og_title', 'og_description', 'og_image_url', 'og_image_alt_text', 'og_image_name',
            'tags', 'schema_rating', 'schema_reviews_count', 'created_by', 'updated_by'
        ]));

        // Refresh secondary keywords if provided
        if ($request->has('secondary_keywords')) {
            SeoSecondaryKeyword::where('primary_keyword_id', $seo->id)->delete();
            foreach ($request->secondary_keywords as $keyword) {
                SeoSecondaryKeyword::create([
                    'primary_keyword_id' => $seo->id,
                    'secondary_keyword' => $keyword['secondary_keyword'],
                    'monthly_search_volume' => $keyword['monthly_search_volume'],
        ...
        return response()->json($seo->load('secondaryKeywords'));
    }
