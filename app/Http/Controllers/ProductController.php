...
class ProductController extends BaseController
{
	...
	*/
	...
	public function update(Request $request, $productId)
	{
		...
		// Handle FAQs with improved permission check
		if ($request->has('faqs')) {
			...
			// Only check for actual FAQ modifications if user doesn't have content permissions
			if (!$canModifyContent) {
				$hasActualFaqChanges = false;
				
				if (is_array($faqs) && !empty($faqs)) {
					$existingFaqs = Faq::where('product_id', $product->id)->get()->keyBy('id');
					
					foreach ($faqs as $faqData) {
						// Check for new FAQs (no ID) with actual content
						if (empty($faqData['id']) && (!empty($faqData['question']) || !empty($faqData['answer']))) {
							$hasActualFaqChanges = true;
							break;
						}
						
						// Check for existing FAQ modifications
						if (!empty($faqData['id']) && $existingFaqs->has($faqData['id'])) {
							$existingFaq = $existingFaqs->get($faqData['id']);
							if ($existingFaq->question !== ($faqData['question'] ?? '') || 
			...
			// Process FAQs if user has permission or no actual changes detected
			if ($canModifyContent || !$hasActualFaqChanges) {
				/* Validate that faqs is an array */
				if (!is_array($faqs)) {
					return response()->json([
						'success' => false,
		...
		/* Handle benefits_features field with improved permission check */
		if ($request->has('benefits_features')) {
			$benefitsFeaturesInput = $request->input('benefits_features');
			
			// Only check permissions if user doesn't have content permissions
			if (!$canModifyContent) {
				$hasActualBenefitsChanges = false;
				
				// Get current benefits_features
				$currentBenefitsFeatures = json_decode($product->benefits_features, true) ?: [];
				
				// Process input to compare
				$newBenefitsFeatures = [];
				if (is_string($benefitsFeaturesInput)) {
					$decoded = json_decode($benefitsFeaturesInput, true);
					if (json_last_error() === JSON_ERROR_NONE) {
						$newBenefitsFeatures = $decoded;
					}
				} elseif (is_array($benefitsFeaturesInput)) {
					$newBenefitsFeatures = $benefitsFeaturesInput;
				}
				
				// Compare current vs new benefits (simple comparison)
				if (!empty($newBenefitsFeatures) && $newBenefitsFeatures !== $currentBenefitsFeatures) {
					$hasActualBenefitsChanges = true;
				}
				
				if ($hasActualBenefitsChanges) {
					return response()->json([
						'success' => false,
			...
			// Process benefits_features if user has permission or no actual changes
			if ($canModifyContent) {
				/* Decode existing benefits_features if available */
				$existingBenefits = json_decode($product->benefits_features, true);
		...
		/* Handle description field with improved permission check */
		if ($request->has('description')) {
			$descriptionInput = $request->input('description');
			
			// Only check permissions if user doesn't have content permissions and is actually changing description
			if (!$canModifyContent) {
				$hasDescriptionChanges = !empty($descriptionInput) && trim($descriptionInput) !== trim($product->description ?? '');
			...
			// Process description if user has permission or no actual changes
			if ($canModifyContent) {
