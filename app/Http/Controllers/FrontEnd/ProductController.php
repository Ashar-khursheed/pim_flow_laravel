...

{
    ...
    // {
    //           $productInput = $request->input('id');  // Can be ID or slug or null

    //     // Base query builder for products with relations
    //     $query = Product::with([
    //         'categories',
    //         'brand',
    //         'productSuppliers',
    ...
    //     if ($productInput) {
    //         if (is_numeric($productInput)) {
    //         // Filter by product ID (single or list with that ID)
    //         $query->where('id', (int) $productInput);
    //     } else {
    //         // Filter by slug using whereHas on seoUrl relation
    ...
    //     // Apply other filters if needed (categories, price, etc)
    //     $this->applyFilters($query, $request);

    //     // If $productInput is set and is non-numeric, we expect just one product (by slug)
    //     if ($productInput && !is_numeric($productInput)) {
    //         $product = $query->first();
   ...
   public function getAllPublicProducts(Request $request)
    {
        $productInput = $request->input('id'); // ID or slug
        ...
        // Apply additional filters (implement as needed)
        $this->applyFilters($query, $request);

        // If slug, fetch single product and return
        if ($productInput && !is_numeric($productInput)) {
            $product = $query->first();
        ...
        // Otherwise paginate product list
        $perPage = 30;
        $page = $request->input('page', 1);
        ...
        $products = $query->orderBy($sortBy, 'desc')
                        ->paginate($perPage, ['*'], 'page', $page);

        // Transform each product for response
        $products->getCollection()->transform(function ($product) {
    ...
    /**
     * Transform product model into desired API structure
     */
    protected function transformProduct(Product $product)
    {
        $benefitsFeatures = [];
        if (!empty($product->benefits_features)) {
            $decoded = json_decode($product->benefits_features, true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                $benefitsFeatures = $decoded;
            }
        }
        ...
        $product->categories->each(function ($category) use (&$allCategories, $getParentHierarchy) {
            $allCategories->push(...$getParentHierarchy($category));
            $allCategories->push($category);
        });

        $uniqueCategories = $allCategories->unique('id')->map(function ($cat) {
            return [
                'id' => $cat->id,
                'name' => $cat->name,
                'slug' => $cat->slug,
        ...
        $totalReviews = $product->reviews->count();
        $avgRating = $totalReviews > 0 ? round($product->reviews->avg('star'), 2) : null;
        ...
        if (!empty($product->productAttributes)) {
            $unitsPerCase = collect($product->productAttributes)
                ->first(fn($attr) => $attr->attributeDetails?->name === 'Units per Case');
            $packType = collect($product->productAttributes)
                ->first(fn($attr) => $attr->attributeDetails?->name === 'Pack Type');
        }
        ...
        return [
            ...
            'benefits_features' => $benefitsFeatures,
            'seo_url' => $product->seoUrl->url ?? null,
            'brand' => $product->brand ? [
                'id' => $product->brand->id,
