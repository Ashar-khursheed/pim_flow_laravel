...
class CompareProductController extends Controller
{
    ...
    public function getCompareTableProduct(Request $request)
    {
        ...
        $alternateProduct = AlternateProduct::where('product_id', trim($request->input('product_id')))->orderBy('priority', 'asc')->get();
    
        if (!empty($alternateProduct) && $alternateProduct->count() > 0) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'This Product ID does not have any alternate products',
                'data' => [],
