    ...
    public function getSpecificationFilters(Request $request)
{
        ...
        foreach ($request->filters as $filter) {
            ...
            foreach ($specValues as $value) {
                if (is_array($value) && isset($value['min']) && isset($value['max'])) {
                    ...
                    $cleanedSpecValues[] = $value;
                } else {
                    // Clean the value by removing count information
                    $cleanedValue = $this->cleanFilterValue($value);
                    if (!empty($cleanedValue)) {
                        $cleanedSpecValues[] = $cleanedValue;
                    }
                }
            }
            ...
            // Check if this is a range filter
            $isRangeFilter = false;
            foreach ($cleanedSpecValues as $value) {
                if (is_array($value) && isset($value['min']) && isset($value['max'])) {
                    $isRangeFilter = true;
    ...
    if ($subCategory) {
        ...
        if ($attributeIdsField && !empty($subCategory->$attributeIdsField)) {
            ...
            // Only proceed if we have valid attribute IDs
            if (!empty($attributeIds)) {
                // Get attribute filters for both parent and child category products with counts
                $attributeValuesWithCounts = DB::table('product_attributes as pa')
                    ->join('attributes as at', 'at.id', '=', 'pa.attribute_id')
                    ->join('ec_products as p', 'p.id', '=', 'pa.product_id')
                    ->whereIn('pa.product_id', $allCategoryProductIds)
                    ->whereIn('pa.attribute_id', $attributeIds)
                    ->where('p.status', 'published')
                    ->select('at.name as attribute_name', 'pa.attribute_value', 'at.id as attribute_id', DB::raw('COUNT(DISTINCT pa.product_id) as product_count'))
                    ->groupBy('at.name', 'pa.attribute_value', 'at.id')
                    ->orderBy('product_count', 'desc') // Order by highest count first
                    ->orderBy('pa.attribute_value', 'asc')
                    ->get();
                    
                $debugInfo['attribute_values_count'] = $attributeValuesWithCounts->count();

                // If we have any attribute values
                if ($attributeValuesWithCounts->count() > 0) {
                    $attributeValuesGrouped = $attributeValuesWithCounts->groupBy('attribute_name');

                    // Process attribute filters
                    foreach ($attributeValuesGrouped as $attributeName => $values) {
                        // Get unique values with their counts
                        $valueCountPairs = $values->map(function($item) {
                            return [
                        ...
                        // Check if all values are numeric-like
                        $numericValues = true;
                        $cleanedValuesWithCounts = $valueCountPairs->map(function($item) use ($extractIntegerValue, &$numericValues) {
                            $cleanedVal = $extractIntegerValue($item['value']);
                            if (!is_numeric($cleanedVal)) {
                                $numericValues = false;
                            }
                            return [
                                'original_value' => $item['value'],
                                'cleaned_value' => $cleanedVal,
                                'count' => $item['count']
                            ];
                        });

                        if ($numericValues && $cleanedValuesWithCounts->count() > 2) {
                            // Group by cleaned values and sum counts
                            $groupedByCleanedValue = $cleanedValuesWithCounts->groupBy('cleaned_value')->map(function($group) {
                                return [
                                    'value' => $group->first()['cleaned_value'],
                            ...
                            // Store original mapping for debugging
                            $debugInfo['numeric_values_' . $attributeName] = $groupedByCleanedValue->toArray();

                            // Calculate ranges based on actual data
                            $uniqueValues = $groupedByCleanedValue->pluck('value')->unique()->sort();
                            $chunkCount = min(5, ceil($uniqueValues->count() / 2));
                            $chunkSize = ceil($uniqueValues->count() / $chunkCount);

                            $ranges = $uniqueValues->chunk($chunkSize)->map(function ($chunk) use ($groupedByCleanedValue) {
                                $min = $chunk->first();
                                $max = $chunk->last();
                                
                                // Calculate total count for this range
                                $rangeCount = $groupedByCleanedValue->filter(function($item) use ($min, $max) {
                            ...
                            })->sortByDesc('count')->values()->toArray(); // Sort by highest count first
                            ...
                            // For non-numeric values, sort by count (highest first)
                            $sortedValues = $valueCountPairs->sortByDesc('count')->values()->map(function($item) {
                                return [
                                    'value' => $item['value'],
                                    'count' => $item['count']
                                ];
                            ...
                            $filters[] = [
                                'specification_name' => $attributeName,
                                'specification_type' => 'fixed',
                                'specification_value' => $sortedValues->toArray(),
                            ];
                        }
                    }
                }
    ...
    // Get brands from all products (parent + child categories) with counts
    $brandCounts = DB::table('ec_products as p')
    ...
    $brands = $brandCounts->map(function($brand) {
        return [
            'id' => $brand->id,
            'name' => $brand->name,
    ...
    // Get rating counts
    $ratingCounts = DB::table('ec_reviews as r')
        ->join('ec_products as p', 'p.id', '=', 'r.product_id')
        ->whereIn('r.product_id', $allCategoryProductIds)
        ->where('p.status', 'published')
        ->select(DB::raw('ROUND(AVG(r.star)) as rating'), DB::raw('COUNT(DISTINCT r.product_id) as product_count'))
        ->groupBy(DB::raw('ROUND(AVG(r.star))'))
        ->orderBy('product_count', 'desc')
        ->get();

    $ratingValues = $ratingCounts->map(function($rating) {
        return [
            'value' => (int)$rating->rating,
            'count' => $rating->product_count
        ];
    })->sortByDesc('count')->values();

    // If no ratings found, provide default structure
    if ($ratingValues->isEmpty()) {
        $ratingValues = collect([5, 4, 3, 2, 1])->map(function($rating) {
            return [
                'value' => $rating,
                'count' => 0
            ];
    ...
    $ratingFilter = [
        'filter_name' => 'Rating',
        'filter_type' => 'rating',
        'filter_values' => $ratingValues->toArray(),
    ];
...
/**
 * Clean filter value by removing count information
 * Handles formats like "Value (123)" or "Value - 123" etc.
 */
private function cleanFilterValue($value)
{
    ...
    // Remove count information in parentheses like "Red (25)"
    $cleaned = preg_replace('/\s*\(\d+\)\s*$/', '', $value);
    
    // Remove count information with dash like "Red - 25"
    $cleaned = preg_replace('/\s*-\s*\d+\s*$/', '', $cleaned);
    
    // Remove count information with pipe like "Red | 25"
    $cleaned = preg_replace('/\s*\|\s*\d+\s*$/', '', $cleaned);
    
    // Remove count information with colon like "Red: 25"
    $cleaned = preg_replace('/\s*:\s*\d+\s*$/', '', $cleaned);
