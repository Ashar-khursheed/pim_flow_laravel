...
public function getSpecificationFilters1(Request $request)
{
    ...
    // FIXED: Get all category and child category product IDs
    $allCategoryProductIds = $this->getAllCategoryProductIds($category->id);

    if (empty($allCategoryProductIds)) {
        return $this->getEmptyResponse();
    }

    // ... [rest of your convertAttributeValue function and other logic remains the same] ...

    $convertAttributeValue = function($attributeName, $originalValue) use ($categoryMeasurementPriorities) {
        // [Your existing convertAttributeValue logic - keeping it as is]
        $originalValue = trim($originalValue);
    ...
    // FIXED: Get all category product IDs using the corrected method
    $allCategoryProductIds = $this->getAllCategoryProductIds($category->id);
    ...
    // [Rest of your code for fetching products, building filters, etc. remains the same...]
    // The main fix was in getAllCategoryProductIds method below
}

/**
 * FIXED: Get all product IDs for a category including child categories
 */
private function getAllCategoryProductIds($categoryId)
{
    // Get products from current category using the pivot table
    ...
    $allChildCategoryIds = $this->getAllChildCategoryIds($categoryId);
    
    // Get products from all child categories using the pivot table
    $childProductIds = [];
...
/**
 * FIXED: Recursively get all child category IDs
 */
private function getAllChildCategoryIds($categoryId)
{
    ...
    if (!empty($directChildren)) {
        $childCategoryIds = array_merge($childCategoryIds, $directChildren);
        
        // Recursively get children of children
        foreach ($directChildren as $childId) {
            $grandChildren = $this->getAllChildCategoryIds($childId);
            $childCategoryIds = array_merge($childCategoryIds, $grandChildren);
        }
    }
    
    return array_unique($childCategoryIds);
}
// public function getSpecificationFilters1(Request $request)
// {
//     // Validation
