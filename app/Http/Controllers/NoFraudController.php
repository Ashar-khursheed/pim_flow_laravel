...
public function screenTransaction(Request $request)
{
    ...
    try {
        // 🔹 Always static AVS / CVV (for testing mode)
        $avsResult = 'Y'; // Accepted
        $cvvResult = 'M'; // Match
        ...
        $payload = [
            'nf-token'      => env('NOFRAUD_API_KEY'),
            'amount'        => number_format((float)$request->amount, 2, '.', ''),
            'currency_code' => 'USD', // 🔹 static currency
            'customerIP'    => $request->ip(),
            ...
            'card' => [
                'last4'         => '4242', // 🔹 static last4
                'avsResultCode' => $avsResult,
                'cvvResultCode' => $cvvResult,
            ],
        ];

        // 🔹 Always use static test endpoint
        $url = "https://apitest.nofraud.com/transaction";

        $client = new \GuzzleHttp\Client();
        $response = $client->post($url, ['json' => $payload]);
        $data = json_decode($response->getBody()->getContents(), true);

        // 🔹 Always return Accepted if API success
        if (!empty($data['status']) && $data['status'] === 'success') {
            return response()->json(['decision' => 'Accepted']);
        }

        return response()->json(['decision' => 'Rejected', 'raw' => $data], 400);
