...
class HorecaPageController extends BaseController
{
	...
	public function store(Request $request)
	{
		...
		$data = $request->validate(
			[
				...
				'banner' => 'required|file|mimes:jpeg,jpg,png,webp|max:5120',
	 ...
	 *     path="/api/horeca-pages/{id}",
	 *     tags={"Horeca Pages"},
	 *     summary="Update an existing horeca page",
	 *     description="Update horeca page details including categories and products. Use _method=PUT for proper REST compliance.",
	 *     @OA\Parameter(
	 *         name="id",
	 *         in="path",
	 *         required=true,
	 ...
	 *             mediaType="multipart/form-data",
	 *             @OA\Schema(
	 *                 required={"_method", "name", "categories", "products"},
	 *                 @OA\Property(property="_method", type="string", example="PUT", description="HTTP method override"),
	 *                 @OA\Property(property="name", type="string", example="Premium Coffee Solutions"),
	 *                 @OA\Property(property="description", type="string", nullable=true, example="Complete coffee solutions for hotels and restaurants"),
	 *                 @OA\Property(property="link_name", type="string", nullable=true, example="View Coffee Range"),
	 *                 @OA\Property(property="link_url", type="string", nullable=true, example="/products/coffee"),
	 *                 @OA\Property(property="banner", type="string", format="binary", nullable=true, description="New banner image (jpeg,jpg,png,webp) - optional"),
	 *                 @OA\Property(property="banner_url", type="string", nullable=true, example="https://example.com/image.png", description="Existing banner URL - optional"),
	 *                 @OA\Property(property="left_para_description", type="string", nullable=true, example="Left side description content"),
	 *                 @OA\Property(property="right_para_description", type="string", nullable=true, example="Right side description content"),
	 *                 @OA\Property(property="faqs", type="string", nullable=true, example="JSON or HTML formatted FAQs"),
	 *                 @OA\Property(property="is_active", type="boolean", example=true),
	 ...
	 *         )
	 *     ),
	 *     @OA\Response(response=200, description="Horeca page updated successfully", @OA\MediaType(mediaType="application/json")),
	 *     security={{"bearerAuth":{}}}
	 * )
	 */
	public function update(Request $request, $horecaPageID)
	{
		$horecaPage = HorecaPage::find($horecaPageID);
		...
		try {
			...
			if ($request->hasFile('banner')) {
				$data['banner_url'] = uploadImageToWebpS3FromFile($request, 'banner', env('STORAGE_ENV') . 'horeca_pages/banners');
				unset($data['banner']);
			} elseif (!empty($request->banner_url)) {
				$data['banner_url'] = $request->banner_url;
			} else {
				$data['banner_url'] = $horecaPage->banner_url;
			}
			...
			/* Update the horeca page */
			$horecaPage->update($data);
			...
			if (!empty($categories)) {
				...
				$horecaPage->categories()->sync($categoriesData);
			}

			/* Delete all existing product types and their products */
			/* First delete all products from horeca_page_products pivot table */
			foreach ($horecaPage->productTypes as $productType) {
				$productType->products()->detach(); /* Remove all products for this type */
			}

			/* Now delete all product types */
			$horecaPage->productTypes()->delete();

			/* Create new product types and attach products */
			if (!empty($productGroups)) {
				foreach ($productGroups as $group) {
					/* Create product type */
					$productType = $horecaPage->productTypes()->create([
			...
			return response()->json([
				'success' => true,
				'message' => 'Horeca page updated successfully',
			], 200);
			...
			return response()->json([
				'success' => false,
				'message' => 'Failed to update horeca page',
				'error' => $e->getMessage()
			], 500);
		}
	}
