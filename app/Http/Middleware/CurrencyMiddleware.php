...
class CurrencyMiddleware
{
    ...
    private function convertPrice(float $price, array $ctx): float
    {
        ...
        // Step 2: AED hi hai toh conversion nahi
        if ($targetCode === 'AED') {
            return round($priceAfterMargin, $decimals);
        }

        // Step 3: Exchange rates (cached 6 hours, koi DB change nahi)
        $rates      = $this->getExchangeRates();
        $targetRate = $rates[$targetCode] ?? null;

        if (!$targetRate) {
            Log::warning('CurrencyMiddleware: unknown code', ['code' => $targetCode]);
            return round($priceAfterMargin, $decimals);
        }
    ...
    // Exchange rates — open.er-api.com se fetch, 6 hours cache
