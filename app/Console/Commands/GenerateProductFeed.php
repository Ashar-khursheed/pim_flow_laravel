...
class GenerateProductFeed extends Command
{
    ...
    public function handle()
    {
        ...
        // Write XML header (compact, no extra lines)
    ...
    private function buildProductXml($product, $seoDataMap, $attributesMap)
    {
        ...
        $slug = $product->seoProductUrl->url ?? $product->id;
        ...
        // Ultra-compact XML (no whitespace, no line breaks)
        $xml = '<item>';
        ...
        $xml .= '<g:link>' . config('app.url') . '/' . $slug . '</g:link>';
        ...
        // Only 2 attributes (reduced from unlimited)
