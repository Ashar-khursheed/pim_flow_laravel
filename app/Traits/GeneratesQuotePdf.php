...
trait GeneratesQuotePdf
{
	...
	public function generateQuotePdfParams($quoteId)
	{
		...
		$totalInWords = match (config('app.website')) {
			'UAE', 'UAE_T' => convertNumberToWords($total, 'AED', 'Fils'),
			'US', 'US_T' => convertNumberToWords($total, 'U.S. Dollars', 'Cents'),
			'SA' => convertNumberToWords($total, 'Saudi Riyals', 'Halalas'),
			default => convertNumberToWords($total, 'U.S. Dollars', 'Cents'),
		};

		$payNowUrl = config('app.url') . '/download-quotation/' . $quote->id;
		$siteName = config('app.website');

		$beneficiaryAddress = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? '' : '8800 BISSONNET ST STE A, HOUSTON TX 77074-2435';
		$accountNo = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? '1015 9086 9400 1' : '6130 9953 3';
		$bankName = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? 'Emirates NBD' : 'JP Morgan Chase Bank';
		$routingCode = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? '' : '1110 0061 4';
		$ibanNumber = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? 'AE48 0260 0010 1590 8694 001' : '';
		$swiftCode = in_array(config('app.website'), ['UAE', 'UAE_T', 'SA']) ? 'EBILAEADXX' : '';
