...
def process_index(i, imp_df):
    ...
    if isinstance(answer, pd.DataFrame) and not answer.empty and query_attrs:
        def add_similarity(row):
            candidate_attrs = ast.literal_eval(row["allattributes_dict"]) if isinstance(row["allattributes_dict"], str) else row["allattributes_dict"]
    ...
    if isinstance(answer, pd.DataFrame):
        ...
        answer['product_alternate_id'] = answer['product_id'] if 'product_id' in answer.columns else None
        answer['priority'] = answer['matched_attributes_count']
        answer['similarity'] = answer['similarity_percentage']
        cols_to_keep = ['id', 'product_alternate_id', 'priority', 'similarity']
...
def save_alternates_to_db(answer_df: pd.DataFrame, db_config: dict):
    ...
    (product_id, product_alternate_id, priority, similarity, created_at, updated_at)
...
def process_and_save(i, imp_df, db_config):
    try:
...
LEFT JOIN cte_product_type pt ON c.product_id = pt.product_id
    ...
    # --- INPUT FOR SINGLE ID OR ALL ---
    user_input = input("Enter product ID (or 'all' for all products): ").strip().lower()
    if user_input == 'all':
        product_id_list = imp_df['product_id'].tolist()
        logging.info("Processing all products in the database.")
    else:
        try:
            product_id_list = [int(user_input)]
            logging.info(f"Processing single product ID: {product_id_list}")
        except ValueError:
            logging.error("Invalid input. Please enter a valid product ID or 'all'.")
            return
    ...
    max_workers = min(len(product_indices_to_process), os.cpu_count() or 4)  # Limit workers to CPU count or 4
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_and_save, i, imp_df, db_config) for i in product_indices_to_process]
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Processing products"):
            try:
                future.result()
            except Exception as e:
