...
TITLE_TEMPLATES = [
    "{pk} Available in UAE Now",
    "Shop {pk} in UAE Today",
    "Your UAE Source for {pk}"
]
META_TEMPLATES = [
    "{usp} on {pk} in UAE Excellence",
    "{pk} with {usp} Quality in UAE",
    "Exclusive {pk} with {usp} in UAE"
]
OG_TEMPLATES = [
    "{pk} with {usp} in UAE Quality",
    "{usp} for {pk} in UAE Excellence",
    "{pk} — {usp} Excellence in UAE"
]
BENEFITS = ["Fast UAE Delivery", "Unbeatable UAE Deals", "Top Performance", "Exclusive UAE Savings"]
CTAS = {
    "shop": ["Shop Now in UAE!", "Order Today in UAE!", "Grab Yours in UAE!"],
    "explore": ["Explore More in UAE", "Discover Now in UAE", "Learn More in UAE"],
    "read": ["Read More in UAE", "Dive In in UAE", "Learn More in UAE"]
}
...
def generate_meta_description(pk, rtype):
    ...
    if rtype=="Blog":
        return f"Explore {pk} insights in UAE, with expert tips and in-depth analysis from trusted UAE resources. {cta} to learn more."[:160]
    return f"Premium {pk} in UAE, built for durability and excellence. Enjoy {pick(BENEFITS)} with every purchase. {cta}"[:160]
def generate_og_description(meta_desc, rtype, pk):
    cta = pick(CTAS["explore"]) if rtype=="Blog" else pick(CTAS["shop"])
    if rtype=="Blog":
        return f"Trusted UAE source for {pk} insights. Dive into expert tips, analysis, and resources to elevate your knowledge in UAE. {cta}"[:200]
    return f"Top choice for {pk} in UAE! Premium quality, lasting durability, and {pick(BENEFITS)}. {cta} to experience UAE excellence."[:200]
