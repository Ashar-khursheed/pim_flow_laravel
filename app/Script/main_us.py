...
META_TEMPLATES = [
    "{usp}: {pk} | {brand}",
    "{pk} — {brand} | {usp}",
    "{brand} Presents {pk} — {usp}"
]
OG_TEMPLATES = [
    "{pk} — {usp}",
    "{usp} for {pk}",
    "{pk} — Top Choice by {brand}"
]
...
def generate_meta_description(pk, rtype):
    ...
    if rtype=="Blog": return f"Dive into {pk} insights—expert analysis inside. {cta}"[:160]
    return f"Premium {pk} built for lasting durability. {cta}"[:160]
def generate_og_description(meta_desc, rtype, pk):
    cta = pick(CTAS["explore"]) if rtype=="Blog" else pick(CTAS["shop"])
    if rtype=="Blog":
        part = meta_desc.split(".")[0]; return f"Trusted source for {pk}—{part}. {cta}"[:200]
    echo = "top performance" if "durability" in meta_desc else meta_desc.split(".")[0]
    return f"Top choice for {pk}! {echo.capitalize()}. {cta}"[:200]
...
def enrich(record):
    ...
    if not orig["meta_title"]:     rec["meta_title"] = generate_meta_title(pk, brand)
    if not orig["og_title"]:       rec["og_title"] = generate_og_title(pk, brand)
